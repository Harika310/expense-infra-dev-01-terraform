...
# null resource to configure backend with remote provisioner
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["expense-${var.environment}.${var.zone_name}"] # expense-dev-daws-81s.online
    }
  }
}